...
/// Used to calculate \c dateWhenReady when sending sysex commands.
- (NSTimeInterval) processingDelayForSysex: (NSData *)sysex;

/// Sends specified the sysex message on its way to the external device.
/// Called by \c handleSysex after volume-related preprocessing, and called instead of handleSysex
